...
const useEventsService = () => {
    ...
    //Search Event
    const useHandleSearchEventService = (eventType,culture,isFree,eventCat,dateFilter,query,lat,lng,cultureGroup,neighborhood,organizationProfileType,sortBy) => {

        const handleSearchEventRequest = (eventType,culture,isFree,eventCat,dateFilter,query,lat,lng,cultureGroup,neighborhood,organizationProfileType,sortBy) => {
            const page = 1
            const limit = 99
            let parms={
                page:page,
                ...
                latitude:lat.value,
                longitude:lng.value,
                organizationProfileType:organizationProfileType.value ? organizationProfileType.value : 'organizations',
                sortBy:sortBy.value
            }
        ...
        return useQuery(
            ['search-event',eventType,culture,isFree,eventCat,dateFilter,query,lat,lng,cultureGroup,neighborhood,organizationProfileType,sortBy],
            () => handleSearchEventRequest(eventType,culture,isFree,eventCat,dateFilter,query,lat,lng,cultureGroup,neighborhood,organizationProfileType,sortBy),
            {
                onError,
                retry:1,
    ...
    const useHandlePurchaseTicketsService = () => {
        ...
        const onSuccess = (response,ticketData) => {
            ...
            if(TokenService?.getUser()){
                ...
                if(userDetails?.role == ROLES.GUEST){
            ...
            if(response?.data?.status == true){
                // useToaster('success',response?.data?.message,5000);
                window.location.href = BASE_URL+'?payment-response=success&action=booking&event='+createSeoFriendlyUrl(ticketData?.bookedEventTitle)+'&ticket='+ticketData?.totalTicketBooked
                
            }else{
                useToaster('error',response?.data?.message,5000)
            }
            ...
            if(typeof response?.data?.data === "string"){
                ...
                // window.location.href = BASE_URL+'?stripe-response=success&action=booking&event='+createSeoFriendlyUrl(ticketData?.bookedEventTitle)+'&ticket='+ticketData?.totalTicketBooked
        ...
        const onError = (error) => {
            ...
            useToaster('error',viewError(error.response.data.message),5000)
