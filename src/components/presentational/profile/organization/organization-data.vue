  ...
  <div class="" v-if="userRole !== 'Event Planner'">
    ...
    <div class="organization-profile-tab">
      ...
      <ul class="nav nav-tabs mb-3" id="ex1" role="tablist">
        ...
        <li class="nav-item" role="presentation">
          <router-link class="nav-link " :to="{name:ROUTES.ORGANIZER, params:{organizerId:userData._id}}">My Personal Dashboard</router-link>
        </li>
        <li class="nav-item" role="presentation">
          <router-link class="nav-link active" :to="{name:ROUTES.ORGANIZATION, params:{organizerId:userData._id}}">Business Dashboard</router-link>
        </li>
      </ul>
      <!-- Tabs navs -->
    ...
    <div class="custom-left-section">
      <div class="logoinfo w-full" style="margin-top: 50px;margin-bottom: 50px;">
        <div style="max-width: 130px;width: 100%;">
          <img v-if="!userData?.organizationDetails?.profilePicture?.isCompleteUrl" style="width:100%;" class="w-full object-cover lg__mobile:w-32 lg__mobile:h-32 profileimg1"
            :src="userImg(`${userData?.organizationDetails?.profilePicture?.url}`)" alt="Organizer" />

            <img v-if="userData?.organizationDetails?.profilePicture?.isCompleteUrl" style="width:100%;" class="w-full object-cover lg__mobile:w-32 lg__mobile:h-32 profileimg1"
            :src="`${userData?.organizationDetails?.profilePicture?.url}`" alt="Organizer" />
        </div>
        <div class="ml-5">
          <h1 class="mb-2 text-primary__color text-4xl font-bold lg__mobile:text-2xl" v-if="!loader">
            <!-- {{ userData?.role == 'Organizer' || 'Event Planner' ? userData?.organization : `${userData?.firstName}
                        ${userData?.lastName}` }} -->
              {{ userData?.organization != undefined && userData?.organization != '' ? userData?.organization : `` }}
            <span>
          ...
          <p class="text-lg text-center text-primary__color lg__mobile:text-xl mt-1 "
            ...
            >{{
              userData.organizationDescription }}</p>
    ...
    <div class="custom-right-section" style="display:grid;">
      <div class="logoinfo " style="width: 310px;float: right;margin-top: 25px;">
        <Button v-if="isLoggedInUser" button-text="Ticket Orders"
          :click-handler="() => router.push({ name: ROUTES.TICKET_ORDERS, query:{r:ROLES.ORGANIZATION} })"
          custom-classes="!text-heading__color border-2 border-heading__color bg-transparent rounded-[32px] me-3 w-60 " />

        <Button v-if="isLoggedInUser" button-text="Payments" :click-handler="() => router.push({ name: ROUTES.PAYMENTS })"
      ...
      <div class="logoinfo " style="width: 310px;float: right;margin-top: 40px;">
        <Button v-if="isLoggedInUser && isProfileCompleted" button-text="Create an Event"
          :click-handler="() => router.push({ name: ROUTES.ADD_EVENTS, query:{r:ROLES.ORGANIZATION} })"
        ...
        <Button v-if="isLoggedInUser" button-text="Manage Events"
          :click-handler="() => router.push({ name: ROUTES.MANAGE_EVENTS, query:{r:ROLES.ORGANIZATION} })"
          custom-classes="!text-heading__color border-2 border-heading__color bg-transparent rounded-[32px] " />
...
const payoutclick = () => {
  ApiClient.put('profile-settings/payout?role=Organization&userId=' + tokenService.getUser()?._id, {}).then(res => {
    if (res.data) {
...
const goToSettings = () => {
  router.push({ name: ROUTES.EDITPROFILE, query:{profile:'Organization',c:isProfileCompleted.value} })
}
...
const updateCountryCode = async () => {
  ...
  ApiClient.put('profile-settings/country-code?role=Organization&userId=' + tokenService.getUser()?._id, { country: fieldDetails.country }).then(res => {
...
const userImg = (img) => {
  ...
  }else if(img == 'placeholder_org.png'){
    value = '/placeholder_org.png'

  }else if(img == 'placeholder.png'){
    value = '/placeholder_org.png'
  }
  return value
}
...
const getUser = () => {
  ...
  ApiClient.get(`profile-settings/detail?userId=${organizerId ? organizerId : userData?.value?._id}`).then(res => {
    ...
    stripeCompletedStatus.value = userData.value?.stripeConnectCompleted;
  })
}
...
watchEffect(async () => {
  let addressData = await useReverseGeocoding(userData?.value?.organizationDetails?.location?.coordinates[1], userData?.value?.organizationDetails?.location?.coordinates[0])
