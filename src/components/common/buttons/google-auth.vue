<template>
  <!-- <span class="g-input-with-google text-center mt-3 mb-5">
      <ul class="d-flex justify-content-center mt-3 gap-4 ">
          <li> -->
            <!-- <button type="button" @click="emit('google-callback')" -->
                    <!-- > -->
                    <a href="javascript:;" @click="emit('google-callback')"><img  :src="deviconGoogle" alt="img" class="google"></a>
              ...
              <!-- <font-awesome-icon class="text-2xl text-[#47A8DE]" icon="fa-brands fa-google"  /> -->
              <!-- {{ buttonText }} -->
            <!-- </button> -->
          <!-- </li>
      </ul>
...
<script setup>
  import deviconGoogle from "@/assets/login/devicon_google.svg";

  defineProps({
    buttonText:String
  })
  const emit = defineEmits(['google-callback'])
</script>
